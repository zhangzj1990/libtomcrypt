...
static void ge_tobytes(unsigned char *,const ge_p2 *);
static void ge_p3_tobytes(unsigned char *,const ge_p3 *);
static int ge_frombytes_negate_vartime(ge_p3 *,const unsigned char *);
...
static void ge_scalarmult_base(ge_p3 *,const unsigned char *);
static void ge_double_scalarmult_vartime(ge_p2 *,const unsigned char *,const ge_p3 *,const unsigned char *);
...
static void sc_reduce(unsigned char *);
static void sc_muladd(unsigned char *,const unsigned char *,const unsigned char *,const unsigned char *);
...
)
{
  ...
  XMEMMOVE(pkcopy,pk,32);
  XMEMMOVE(rcopy,sm,32);
  XMEMMOVE(scopy,sm + 32,32);

  XMEMMOVE(m,sm,smlen);
  XMEMMOVE(m + 32,pkcopy,32);
  ...
  if (crypto_verify_32(rcheck,rcopy) == 0) {
    XMEMMOVE(m,m + 64,smlen - 64);
    XMEMSET(m + smlen - 64,0,64);
    *mlen = smlen - 64;
    return CRYPT_OK;
  }
